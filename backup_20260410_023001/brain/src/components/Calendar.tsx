...
export default function Calendar() {
  ...
  const selectedBookings = selected ? (bookings[selected] || 0) : 0;
  ...
  const toggleBooking = async (ev: CalEvent) => {
    const id = String(ev.id);
    const newValue = ev.booking_enabled === 'yes' ? 'no' : 'yes';
    setToggling(id);
    try {
      await fetch('/api/events/toggle-booking', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id, booking_enabled: newValue }),
      });
      setEvents(prev => prev.map(e => 
        String(e.id) === id ? { ...e, booking_enabled: newValue } : e
      ));
    } catch (err) {
    ...
    <div style={{ marginTop: '16px' }}>
      <button
        ...
        style={{
          ...
          borderRadius: '6px', color: '#9ba0b4', fontSize: '12px', fontWeight: 600,
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: '2px', textAlign: 'center' }}>
              ...
              {cells.map((d, i) => (
                <div
                  ...
                  style={{
                    fontSize: '11px', padding: '3px 1px', borderRadius: '4px', textAlign: 'center',
            ...
            <div style={{ marginTop: '12px' }}>
              ...
              {selectedEvents.length === 0 ? (
                ...
                selectedEvents.map((ev, i) => {
                  ...
                  const bookingOn = ev.booking_enabled === 'yes';
                  const isToggling = toggling === String(ev.id);
                  return (
                    <div key={i} style={{
                      background: 'rgba(0,91,170,0.12)', border: '1px solid rgba(0,91,170,0.25)',
                      borderRadius: '8px', padding: '10px', marginBottom: '8px',
                    }}>
                      {tag && (
                        <div style={{
                          display: 'inline-block', fontSize: '9px', fontWeight: 700, color: tagColor,
                          background: `${tagColor}22`, border: `1px solid ${tagColor}44`,
                          borderRadius: '4px', padding: '1px 6px', marginBottom: '6px',
                      ...
                      <div style={{ display: 'flex', gap: '6px', marginBottom: '8px', flexWrap: 'wrap' }}>
                        <div style={{
                          flex: 1,
                          background: selectedBookings > 0 ? 'rgba(34,197,94,0.1)' : 'rgba(59,130,246,0.1)',
                          border: `1px solid ${selectedBookings > 0 ? 'rgba(34,197,94,0.2)' : 'rgba(59,130,246,0.2)'}`,
                          borderRadius: '6px', padding: '6px 8px', minWidth: '70px',
                        }}>
                          <div style={{ fontSize: '9px', color: '#4a5068', marginBottom: '2px' }}>🪑 Bookings</div>
                          <div style={{ fontSize: '13px', fontWeight: 700, color: selectedBookings > 0 ? '#22c55e' : '#3b82f6' }}>
                            {selectedBookings > 0 ? `${selectedBookings} guests` : '0'}
                          </div>
                        </div>
                        <div style={{
                      ...
                      <div style={{
                        ...
                        background: bookingOn ? 'rgba(34,197,94,0.08)' : 'rgba(100,100,100,0.08)',
                        border: `1px solid ${bookingOn ? 'rgba(34,197,94,0.2)' : 'rgba(100,100,100,0.2)'}`,
                        borderRadius: '6px',
                      }}>
                        <span style={{ fontSize: '10px', color: bookingOn ? '#22c55e' : '#7b8099', fontWeight: 600 }}>
                          🎫 Table Booking {bookingOn ? 'ON' : 'OFF'}
                        </span>
                        <button
                          onClick={() => toggleBooking(ev)}
                          disabled={isToggling}
                          style={{
                            position: 'relative',
                            width: '36px', height: '18px',
                            borderRadius: '9px', border: 'none', cursor: isToggling ? 'wait' : 'pointer',
                            background: bookingOn ? '#22c55e' : '#4a5068',
                            transition: 'background 0.2s',
                          }}
                        >
                          <span style={{
                            position: 'absolute', top: '2px',
                            left: bookingOn ? '20px' : '2px',
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        ...
                        {bookingOn && ev.btn_en && ev.link && (
                          <a href={ev.link} target="_blank" rel="noopener noreferrer" style={{
                            fontSize: '9px', fontWeight: 600, color: '#3b82f6', textDecoration: 'none',
