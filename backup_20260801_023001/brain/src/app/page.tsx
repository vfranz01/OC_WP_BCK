...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    fetch('/api/memorymd').then(r => r.json()).then(d => setMemory(d.content || ''));
  }, []);
        ...
        <Card title="System Status">
          {status ? (
            <dl className="text-sm space-y-1 text-gray-300">
              ...
              <div className="flex justify-between"><dt>Server Uptime</dt><dd>{Math.floor(Number(status.uptime) / 3600)}h {Math.floor((Number(status.uptime) % 3600) / 60)}m</dd></div>
              <div className="flex justify-between"><dt>OpenClaw Uptime</dt><dd>{String(status.openclawUptime || 'N/A')}</dd></div>
