...
# Count the same top-level daily/support files that the validation loop scans.
# The old recursive count included dreaming/archive/*.md and made the summary
# claim it had checked hundreds of files that the loop intentionally skipped.
...
STALE_DAYS=7
# Daily memory filenames and indexes use UTC; keep age reporting on the same
# clock so a host timezone change cannot shift the stale-file boundary.
TODAY_UNIX=$(date -u +%s)
...
for file in "$MEMORY_DIR"/*.md; do
  ...
  # Check for YAML frontmatter (---...---). Locate the closing delimiter
  # explicitly after line 1: grepping the first 20 lines used to match the
  # opening delimiter itself, so a file with an unterminated frontmatter block
  # was incorrectly accepted. Using the actual delimiter line also avoids the
  # old fixed `tail -n +5` assumption when frontmatter has more than 4 lines.
  if head -1 "$file" | grep -q "^---$"; then
    FRONTMATTER_END=$(awk 'NR > 1 && $0 == "---" { print NR; exit }' "$file")
    if [ -n "$FRONTMATTER_END" ]; then
      VALID_FILES=$((VALID_FILES + 1))

      # Check if file has content beyond the closing frontmatter delimiter.
      total_lines=$(wc -l < "$file")
      body_line_count=$((total_lines - FRONTMATTER_END))
      if [ "$body_line_count" -lt 2 ]; then
        ...
        # Non-fatal: a short body is a warning, not a structural error. Making
        # it non-zero used to trip health-quick-check.sh's auto-repair on every
        # run, even though fix-memory-frontmatter.sh can never fix body length.
      else
        echo "✅ $filename"
      fi
      ...
      fm_block="$(awk -v end="$FRONTMATTER_END" 'NR > 1 && NR < end {print}' "$file")"
  ...
  # Check for stale files (no modifications in N days). The scan is
  # top-level-only, matching TOTAL_FILES and the validation loop above.
...
if [ "$EXIT_CODE" -eq 0 ]; then
  ...
  if [ "$MISSING_BODY" -gt 0 ]; then
    # Structure is valid; only non-fatal body-length warnings remain. Exit 0 so
    # callers (health-quick-check.sh) don't trigger a futile auto-repair loop.
    echo "✅ All memory files structurally valid ($MISSING_BODY non-fatal body warning(s))"
  else
