...
for i in $(seq 0 $((DAYS_TO_REVIEW - 1))); do
  ...
  if [ -f "$LOG_FILE" ]; then
    ...
    # shell — a pipe would run it in a subshell and lose the found_learnings
    # flag, making the script always report "no learnings found".
    while IFS= read -r line; do
      echo "  - **Potential Error/Issue:** $line" >> "$DISTILLATION_OUTPUT"
      found_learnings=true
    done < <(grep -E 'ERROR|FAIL|WARN|ERROR:|FATAL|exception|Traceback' "$LOG_FILE")
    ...
    while IFS= read -r line; do
      ...
      found_learnings=true
    done < <(grep -E 'scripts/check_blog_posts.py|validate_backup.sh|openclaw gateway status unclear|curl allowedDomains may be missing' "$LOG_FILE")
    ...
    done < <(grep -E '✅ Backup valid|✅ curl allowedDomains configured|✅ All critical files present|✅ OpenClaw Gateway healthy' "$LOG_FILE")
