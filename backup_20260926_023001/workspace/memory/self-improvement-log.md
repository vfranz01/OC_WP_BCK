...
## 2026-09-06 — Year-agnostic daily-file pattern in sync-learnings.sh

1. **Found:** `sync-learnings.sh` used `find -name "2026-*.md"` — hardcoded year. On 2027-01-01 the scan would silently match zero files and report "No recent memory files found", silently killing the learnings pipeline (it feeds the daily status summary's distillation step).
2. **Fix:** Replaced with year-agnostic `[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9].md` (exactly matches the YYYY-MM-DD.md daily naming convention, won't match template.md/index.md/etc). Verified: `bash -n` clean; simulated 2027-01-01 file matched by new pattern, missed by old; real `--days 7` run works.
3. Audited all other scripts for hardcoded years: remaining `2026` refs are legit (docs, example filenames, historical archive frontmatter).

## 2026-09-07 — DREAMS.md growth rotation (repo bloat fix)

1. **Found:** `DREAMS.md` is an append-only dream diary written daily by the dreaming runs and committed whole on every workspace snapshot. It had grown to 185KB / 2692 lines, with 11 full-size blobs (~1.8MB) already in git history — and grows linearly forever.
2. **Fix:** New `scripts/rotate_dreams.py` — archives diary entries older than 60 days into per-month files under `memory/dreaming/archive/` (e.g. `dreams-2026-04.md`), preserving the `openclaw:dreaming:*` marker structure the dreaming system expects. Safety: parse-fail = no-op, timestamped backup to gitignored `memory/.backups/`, idempotent, `--dry-run` flag. Wired into `daily-status-summary.sh` behind a >50KB size guard; logs to the daily file only when it actually archives or fails.
3. **Result:** First run archived 156 entries (Apr–Jul 2026), 185676 → 74474 bytes. Verified markers intact, idempotent second run no-ops, `bash -n` clean, daily-status-summary integration works.

## 2026-09-08 — Make memory frontmatter repair self-healing for malformed tags

**Found:** The health check auto-ran `fix-memory-frontmatter.sh` after a validation failure, but the fixer only repaired missing frontmatter. A malformed historical `tags:` flow-list in `memory/2026-09-06.md` therefore survived every run and kept health degraded.

**Changed:** Extended `scripts/fix-memory-frontmatter.sh` with a narrow, backup-safe repair for malformed quoted tag lists. It only edits a `tags: [...]` line inside valid frontmatter when the quote-aware check detects the known malformed form; valid YAML remains unchanged. The existing missing-frontmatter path is untouched.

**Verified:** `bash -n scripts/fix-memory-frontmatter.sh`; fixer repaired the one offending file and created a backup; `validate-memory-files.sh` now exits 0 with zero malformed tag lists.

## 2026-09-09 — Harden daily memory bootstrap

Updated scripts/ensure_daily_memory.sh to use strict mode and resolve its workspace/create script relative to the script itself. A missing or failed daily-file creation now stops immediately instead of allowing later session checks to run against missing state; the script is also portable within the workspace.

## 2026-09-10 — fix-memory-frontmatter: summary & tag heuristics



## 2026-09-11 — sync-learnings.sh detects real learnings

1. Found: sync-learnings.sh grepped '^- **2026' and '[improvement]' tags that no daily file uses — --auto reported 'no substantial learnings' every run despite a week of real fixes.
2. Fix: word-boundary signal regex (fixed/repaired/hardened/bug/root cause/lesson/...), scans recent sections of memory/self-improvement-log.md (where the substance lives) in addition to daily files, strips ## /abs-path noise, dedupes auto-capture against LEARNINGS.md by fingerprint, idempotent one section/day.
3. Verified: bash -n clean; finds 5 candidates for last 7 days (was 1); --auto appends once, second run no-ops. Commit c0ba1b5.

## 2026-09-12 — Health check no longer blind to dead daily snapshot job

1. Found: The documented daily 02:30 UTC snapshot (HOST-SETUP-CRON.sh job 1) has NEVER run — cron-install.log is from May 11 and the host crontab was never fully installed. It stayed invisible because of two self-masking loops in health-quick-check.sh: (a) validate_backup.sh counts ANY snapshot from the last 24h, and validate_critical_rules.sh fires a "Critical rules validation" snapshot on every validation run — so every health check created its own green-washing snapshot; (b) cron-job validation is skipped when crontab is unavailable (always, inside the container).
2. Fix: health-quick-check.sh now has check #9 "Daily snapshot freshness" that only accepts `snapshot-*-daily_auto_snapshot.tar.gz` files ≤26h old — the exact label the daily job passes, baked into the filename by snapshot.sh. Validation/manual snapshots can't mask it anymore. Failure output includes both fix options (HOST-SETUP-CRON.sh on host, or OpenClaw cron agentTurn).
3. Also: snapshot_log.sh now records the trigger label in last_snapshot.txt (snapshot.sh passes $LABEL through) — forensics for which path created which snapshot.
4. Verified: bash -n clean; with the dead job the check reports the issue (was: false green); after a manual `snapshot.sh daily_auto_snapshot` run it reports fresh. snapshot_log label plumbing verified.
5. ⚠️ OPEN: The actual daily job still needs to be installed — cron tool is restricted inside cron-run sessions and host crontab is outside the container. Either run HOST-SETUP-CRON.sh on the VPS host, or (from a non-cron session) add an OpenClaw cron job: isolated agentTurn, `30 2 * * *` UTC, prompt "run bash /home/node/.openclaw/workspace/scripts/snapshot.sh daily_auto_snapshot", toolsAllow exec. Until then the health check will flag it — that's intended.

## 2026-09-13 — Snapshot-job escalation: both install paths confirmed blocked from inside

1. Followed up on the 2026-09-12 OPEN item (daily 02:30 UTC snapshot never installed).
   Tested both options empirically:
   - OpenClaw cron job: cron tool is restricted in cron-run sessions ("Cron tool is
     restricted to the current cron job") — even though the tool appears in the list.
   - Host crontab via exec host=gateway: gateway runs in the same container (hostname
     8c0f1a1b7ff8), crontab binary not even present. HOST-SETUP-CRON.sh is unreachable.
2. Mitigation: ran `snapshot.sh daily_auto_snapshot` manually — fresh labeled snapshot
   (snapshot-20260913-000041, 2.3M), health check #9 green again, all systems healthy.
3. Escalation: messaged Volker with the exact one-liner (run HOST-SETUP-CRON.sh on the
   VPS host via SSH) — the only remaining path, and it needs host access only he has.
   Until installed: any daily cron session should re-run the manual snapshot as stopgap.

## 2026-09-14 — 2026-09-14 — Automatic snapshot stopgap (no more relying on session memory)

1. Found: The documented stopgap for the never-installed daily 02:30 UTC snapshot job was 'any daily cron session should re-run snapshot.sh manually' — but daily-status-summary did not even run on 09-13, so the chain survived only because the self-improvement session happened to act. One missed session = stale backups with no safety net.
2. Fix: New scripts/snapshot_stopgap.sh — runs snapshot.sh daily_auto_snapshot only when the newest labeled snapshot is >=23h old (STOPGAP_MIN_HOURS overridable). Idempotent (fresh snapshot = no-op), never fatal (exit 0 on failure; health check #9 still reports staleness independently so a silent failure stays visible).
3. Wired into both proven entry points: ensure_daily_memory.sh (runs at the start of EVERY session, after daily-file creation succeeds) and daily-status-summary.sh (logs '📸 Snapshot stopgap fired' to the daily log / incident log on failure).
4. Verified: bash -n clean on all three; first run created snapshot-20260914-000118-daily_auto_snapshot.tar.gz (2.3M, chain preserved); second run no-ops; ensure_daily_memory + daily-status-summary full runs pass with the stopgap integrated.

## 2026-09-15 — Prevent overlapping snapshot stopgap runs

Found: `snapshot_stopgap.sh` is now called from both session bootstrap and daily
status summary. If those entry points overlap, both can pass the freshness check
before either snapshot finishes, creating duplicate archives and unnecessary I/O.

Changed `scripts/snapshot_stopgap.sh` to take a non-blocking `flock` lock before
checking freshness. A concurrent invocation now exits cleanly; the lock is
released automatically when the process exits. Minimal images without `flock`
remain supported with a warning and the previous behavior.

Verified: `bash -n` passes; sequential calls remain idempotent and the lock is
non-blocking.

## 2026-09-16 — Harden memory frontmatter delimiter validation

Found a validator blind spot: validate-memory-files.sh searched the first 20 lines for a closing `---`, which could match the opening delimiter itself and accept unterminated YAML frontmatter. It also assumed the body always started on line 5.

Changed scripts/validate-memory-files.sh to locate the actual closing delimiter after line 1 with awk, derive the body length from that line, and scan tags only inside the true frontmatter block. Verified bash -n, a clean real tag scan (0 malformed tags), and a temporary unterminated-frontmatter fixture returning a validation failure. Existing two minimal-body warnings remain unchanged.

## 2026-09-17 — Make snapshot failure handling observable

Found: scripts/snapshot.sh had `set -e` around a standalone tar command, so a tar failure exited before the custom error branch could run. It could also leave a partial archive behind, making backup diagnosis ambiguous.

Changed: Wrapped tar in an explicit if/then failure branch, emit a clear stderr message, remove any partial archive, and preserve exit code 1 for callers. Verified bash -n and a simulated tar failure: custom message appeared and no partial archive remained.

## 2026-09-18 — Validator scope and UTC consistency

Found that validate-memory-files.sh counted 627 recursive markdown files while its loop only validated 176 top-level files, making the summary misleading. Changed the count to match the actual scan and standardized stale-age arithmetic plus daily status/health timestamps on the UTC clock used by memory filenames. Verified bash -n and the validator now reports 176 files; the two pre-existing minimal-body warnings remain.

## 2026-09-19 — Disk-warning blind spot + validator warning-fatality

1. Found (A): check_disk_space.sh's warning branch (90-94% disk) exited 0, so
   health-quick-check.sh reported '✅ Disk space OK' even with the warning
   threshold exceeded. Warning-level disk pressure was completely invisible.
   Found (B): validate-memory-files.sh set exit 1 for the 2 pre-existing
   'minimal body' *warnings*, so health-quick-check.sh ran its
   fix-memory-frontmatter.sh auto-repair loop every single run — futile, since
   that fixer can never lengthen a body — and reported '⚠️ 1 issue' forever.

2. Fix (A): check_disk_space.sh warning branch now exits 2 (0=ok, 1=critical,
   2=warning). health-quick-check.sh #8 captures the code and reports a
   distinct 'Disk space WARNING' line instead of a bare issue.
   Fix (B): minimal-body no longer sets EXIT_CODE; validator exits 0 when only
   non-fatal body warnings remain, prints 'structurally valid (N non-fatal body
   warning(s))', and still exits 1 for real errors (missing/unterminated
   frontmatter, malformed tags).

3. Verified: bash -n on all three scripts; after the fixes health-quick-check.sh
   returns 0 ('All systems healthy', was 1 every run). Negative test: a file
   with no frontmatter still yields rc=1 and the 'Missing YAML frontmatter'
   error; the test fixture was removed. Disk branches exercised at 27%: rc=0
   normal, rc=2 simulated warn, rc=1 simulated critical.

## 2026-09-20 — Prevent accidental snapshot from backtick in daily-status-summary

1. Found: check_last_snapshot() in scripts/daily-status-summary.sh logged its
   'No record found' warning with the repair command wrapped in BACKTICKS inside
   double quotes:
       echo "... Run  \`bash $WORKSPACE_DIR/scripts/snapshot.sh \"manual_check\"\` manually."
   Inside double quotes that is a command substitution, not text. Whenever
   memory/last_snapshot.txt was missing (fresh restore/clone), the status script
   silently EXECUTED a full snapshot.sh run as a side effect and wrote its
   output into memory/backup_incidents.log instead of a readable hint.

2. Fix: removed the backticks, made the hint plain text
   ('Run: bash ... snapshot.sh "manual_check" manually.') and added a comment
   warning future edits against reintroducing the substitution.

3. Verified: bash -n clean. Sandbox repro with a stub scripts/snapshot.sh:
   old form ran the stub (invoked.log recorded 'SNAPSHOT INJECTED') and logged
   the mangled line 'Run  ✅ Snapshot: fake manually.'; new form executes
   nothing and logs the literal command. Pre-existing backtick grep now only
   matches an unrelated explanatory comment.

## 2026-09-21 — Open-blockers tracker (memory/blockers.md + check-blockers.sh)

1. Found: items waiting on Volker/external fixes lived only in daily memory
   files (e.g. 2026-09-20 ecomunivers post blocked on WP keys + Cloudflare
   SSL fix) and silently went stale — nothing surfaced them day-to-day.

2. Added:
   - `memory/blockers.md` — structured open-blocker list (## date — title,
     status: open|resolved, waiting_on, next). Seeded with the ecomunivers
     blocker. Gets YAML frontmatter auto-repaired like other memory files.
   - `scripts/check-blockers.sh` — parses blockers.md; exit 0 when no open
     or only fresh (<24h) blockers, exit 1 when any open blocker is >24h old.
   - `health-quick-check.sh` check #10 — fresh blockers are informational
     (ℹ️), stale ones count as an issue so blocked work stays heartbeat-visible.

3. Verified: bash -n clean on both scripts; check-blockers.sh correctly
   reports the seeded ecomunivers blocker as 1d old (rc=1); full
   health-quick-check.sh run shows check #10 output as expected.

## 2026-09-21 — Daily self-improvement



## 2026-09-22 — Daily self-improvement context brief (scripts/daily-improve-context.sh)

1. Found: the daily self-improvement cron re-derives its context every run by
   manually grepping self-improvement-log.md, blockers.md, git status, health
   checks, and .learnings/ERRORS.md — token-heavy, inconsistent across runs,
   and easy to repeat an already-done fix.
2. Added: `scripts/daily-improve-context.sh` — one-shot brief printing:
   - last 6 self-improvement entries (dedup guard)
   - open blockers with status/waiting_on/next (HTML-comment examples filtered)
   - git uncommitted count + last commit
   - failing health-quick-check lines
   - ERRORS.md status summary + open-error count
   Always exits 0 so the cron run never fails on a missing file.
3. Wired: AGENTS.md now instructs the daily self-improvement agent to run this
   brief first (AGENTS.md is injected into every session including the cron).
   Note: direct cron-job patch was blocked from the isolated run
   ("Cron tool is restricted to the current cron job"), so AGENTS.md is the
   delivery path.
4. Verified: bash -n clean; live run shows correct entries, the ecomunivers
   blocker, 8 dirty files, stale-blocker health failure, 0 open errors.
   Changes committed (3 commits, latest: 102329f+AGENTS.md edit).

## 2026-09-23 — Blocker nudge delivery (scripts/nudge-blockers.sh + daily brief wiring)

Problem: stale 'waiting_on: user' blockers were flagged only in internal logs (health-quick-check, check-blockers.sh) — Volker never saw them, so the 3-day-old ecomunivers blocker sat unactioned.
Fix:
- New scripts/nudge-blockers.sh: parses memory/blockers.md (skips comment template, tolerant of missing trailing newline), prints ready-to-send reminder texts for open blockers older than a threshold (default 48h; --days N, --all).
- Wired into daily-improve-context.sh as 'Blocker nudges' section + one AGENTS.md line: daily agent delivers one reminder per stale blocker per day via Telegram.
- Verified blocker still real before nudging (WP /users/me -> 401 rest_not_logged_in; https://digital.ecomunivers.com -> 301 downgrade to http).
Follow-up: direct message tool send failed with Telegram 401 Unauthorized (dryRun OK) — bot token/delivery path needs checking; nudge left to auto-delivery.

## 2026-09-24 — Nudge dedup + outcome recording

nudge-blockers.sh now tracks sent/failed per blocker per day (memory/nudge-state.log): successful sends dedup for the day, failed attempts keep retrying. --record flag added; AGENTS.md daily-agent flow updated. Note: Telegram 401 persists (bot token).

## 2026-09-25 — --help



## 2026-09-25 — Telegram bot token health check

- New scripts/check_telegram_token.sh: probes every bot token under channels.telegram.accounts.* via Telegram getMe (free, side-effect-free). Exit 1 on 401 (needs BotFather regen), 2 on config/network problems.
- Wired into health-quick-check.sh as check #11 so a revoked/stale bot token surfaces in every health check + daily brief instead of being discovered days late during failed nudge attempts.
- Context: both 'coder' and 'default' bot tokens currently return 401 Unauthorized — outbound Telegram sends (blocker nudges) fail silently since 2026-09-23. Confirmed via direct getMe probe + message tool attempt (recorded failed in nudge-state.log). Volker action: regenerate tokens via @BotFather and update openclaw.json.




## 2026-09-26 — Nudge state in daily brief

- daily-improve-context.sh now shows today's recorded nudge outcomes from memory/nudge-state.log (section 2c) right under the nudge list.
- On a failed send it prints the recovery path (check_telegram_token.sh probe; record failed via nudge-blockers.sh) so the daily agent stops blindly re-attempting sends that die on 401 bot tokens.
- Also cleaned a bogus --help entry that had been logged by calling log-self-improvement.sh without args.
