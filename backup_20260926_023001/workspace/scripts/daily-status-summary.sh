...
INCIDENT_LOG="$MEMORY_DIR/backup_incidents.log"
  ...
  if [ -f "$INCIDENT_LOG" ]; then
    # Rotate to the previous day's archive ONLY when the file still holds
    # entries from before today's midnight (i.e. the first run after rollover).
    # Without this guard, a second run in the same day would re-run
    # `date -d "yesterday"` and move TODAY's freshly-logged incidents into a
    # wrongly-dated file, then truncate them — losing real incidents.
    let TODAY_MIDNIGHT=$(date -u -d "$(date -u +%F)" +%s 2>/dev/null || date -u +%s)
    if [ -s "$INCIDENT_LOG" ] && [ "$(stat -c %Y "$INCIDENT_LOG" 2>/dev/null || echo 0)" -lt "$TODAY_MIDNIGHT" ]; then
      YESTERDAY=$(date -u -d "yesterday" +%Y-%m-%d)
      mv "$INCIDENT_LOG" "${INCIDENT_LOG%.log}-${YESTERDAY}.log"
      : > "$INCIDENT_LOG"
    fi
    # Otherwise the file already holds today's incidents (or is empty from an
    # earlier run today) — leave it intact so nothing is lost or mislabelled.
  else
...
# Stopgap daily snapshot (guarded, idempotent): keeps the backup chain alive
# until the real host crontab is installed. Runs at most once per 23h window;
# failures are logged to the incident log but never abort this script.
STOPGAP_OUT=$(bash "$WORKSPACE_DIR/scripts/snapshot_stopgap.sh" 2>&1)
if echo "$STOPGAP_OUT" | grep -q '^🔄'; then
  if echo "$STOPGAP_OUT" | grep -q '✅ Snapshot:'; then
    echo "  - **[$TODAY]** 📸 Snapshot stopgap fired: $(echo "$STOPGAP_OUT" | grep '✅ Snapshot:' | head -1 | sed 's/✅ Snapshot: //')" >> "$DAILY_LOG"
  else
    echo "  - **[$TODAY]** ⚠️ Snapshot stopgap failed: $STOPGAP_OUT" >> "$DAILY_LOG"
    echo "$(date -u '+%Y-%m-%d %H:%M UTC') - snapshot_stopgap failed: $STOPGAP_OUT" >> "$INCIDENT_LOG"
  fi
fi
...
check_openclaw_gateway_status() {
  ...
  if ! status_output=$(openclaw gateway status 2>&1); then
    ...
    echo "$(date -u '+%Y-%m-%d %H:%M UTC') - OpenClaw gateway status failed: $status_output" >> "$INCIDENT_LOG"
    return 0
  fi
  ...
  elif grep -q "Connectivity probe: ok" <<< "$status_output"; then
    echo "✅ OpenClaw Gateway reachable and responding (probe ok)"
  else
    echo "⚠️ OpenClaw Gateway status unclear"
    echo "$(date -u '+%Y-%m-%d %H:%M UTC') - OpenClaw gateway status unclear: $status_output" >> "$INCIDENT_LOG"
  fi
}
...
check_last_snapshot() {
  ...
  if [ -f "$last_snapshot_file" ]; then
    ...
    # NOTE: do NOT wrap the command in backticks here — inside double quotes that
    # is a command substitution and would silently run a full snapshot as a
    # side effect, dumping its output into the incident log. Keep it literal.
...
if [ -f "$DAILY_LOG" ]; then
  ...
  echo "" >> "$DAILY_LOG"
  echo "## Status Check — $(date -u '+%H:%M UTC')" >> "$DAILY_LOG"
else
  ...
  content="$content\n## Status Checks — $(date -u '+%H:%M UTC')\n"
  
  echo "$content" > "$DAILY_LOG"
fi

# Auto-refresh memory/index.md when it lags today. The canonical refresh lives in
# daily-heartbeat-routine.sh, but that routine is not scheduled on this host, so
# the index went stale (missing daily logs). This guard makes the index
# self-healing on the very script that creates the daily logs. It is idempotent:
# when the heartbeat routine does run, its own step simply sees a fresh index.
INDEX_FILE="$MEMORY_DIR/index.md"
INDEX_DATE="$(grep -m1 '^date: ' "$INDEX_FILE" 2>/dev/null | sed 's/^date:[[:space:]]*//')"
if [ "$INDEX_DATE" != "$TODAY" ]; then
  # Note: append directly to $DAILY_LOG here — stdout is not yet redirected
  # to the log at this point in the script (that happens in the summary block).
  if python3 "$WORKSPACE_DIR/scripts/update-memory-index.py" >/dev/null 2>&1; then
    echo "  - **[$TODAY]** 🗂️ Refreshed memory index (was $INDEX_DATE, now $TODAY)" >> "$DAILY_LOG"
  else
    echo "  - **[$TODAY]** ⚠️ Memory index refresh failed (update-memory-index.py)" >> "$DAILY_LOG"
  fi
fi

# Keep DREAMS.md from growing forever: rotate entries older than 60 days into
# memory/dreaming/archive/. DREAMS.md is committed whole on every snapshot, so an
# unbounded diary bloats the repo (it hit 185KB / 11 full-size git blobs). Cheap
# and idempotent — we only log when it actually archived something or failed.
DREAMS_FILE="$WORKSPACE_DIR/DREAMS.md"
if [ -f "$DREAMS_FILE" ] && [ "$(stat -c%s "$DREAMS_FILE" 2>/dev/null || echo 0)" -gt 51200 ]; then
  ROT_OUT=$(python3 "$WORKSPACE_DIR/scripts/rotate_dreams.py" --days 60 2>&1)
  ROT_RC=$?
  case "$ROT_RC" in
    0) [ -n "$ROT_OUT" ] && echo "$ROT_OUT" | grep -q '^OK: archived' && \
         echo "  - **[$TODAY]** 🗂️ $ROT_OUT" >> "$DAILY_LOG" ;;
    *) echo "  - **[$TODAY]** ⚠️ DREAMS.md rotation failed: $ROT_OUT" >> "$DAILY_LOG" ;;
  esac
fi
...
if bash "$WORKSPACE_DIR/scripts/health-quick-check.sh" > "$HEALTH_TMP" 2>&1; then
  ...
  echo "$(date -u '+%Y-%m-%d %H:%M UTC') - health-quick-check.sh failed with exit $HEALTH_EXIT; see $HEALTH_LOG" >> "$INCIDENT_LOG"
fi
cat "$HEALTH_TMP"
...
# Run checks and append results
{
  ...
  if [ "$OVERALL_STATUS" = "healthy" ]; then
    log_status "System healthy at $(date -u '+%Y-%m-%d %H:%M UTC')"
  else
    log_status "System degraded at $(date -u '+%Y-%m-%d %H:%M UTC') — review $HEALTH_LOG and $INCIDENT_LOG"
  fi
