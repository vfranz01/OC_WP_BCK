...
# Check OpenClaw gateway status (top-level function for reusability)
...
# Check if sync learnings has been run
if [ ! -f "$WORKSPACE_DIR/memory/.learnings_run" ]; then
  log_status "⚠️  sync-learnings.sh has not been run yet. Run `bash $WORKSPACE_DIR/scripts/sync-learnings.sh --days 7` to capture recent learnings. After the first run, this message will disappear."
  touch "$WORKSPACE_DIR/memory/.learnings_run"
fi

# Check if sync learnings has been run
LEARNINGS_RUN_FILE="$WORKSPACE_DIR/memory/.learnings_run"
...
if [ ! -f "$DAILY_LOG" ]; then
  echo "⚠️ ERROR: Daily log file $DAILY_LOG not created! Check /tmp/create_daily_memory_file.log for details." >> /home/node/.openclaw/workspace/memory/backup_incidents.log
fi
if [ -f "$DAILY_LOG" ]; then
