...
- date: 2026-05-16 title: Daily Heartbeat & Snapshot Status tags: [backup, snapshot, heartbeat] projects: [infrastructure] [score=0.805 recalls=0 avg=0.620 source=memory/2026-05-16.md:2-5]
<!-- openclaw-memory-promotion:memory:memory/2026-05-16.md:6:6 -->
- summary: Snapshot execution and backup validation on May 16th [score=0.805 recalls=0 avg=0.620 source=memory/2026-05-16.md:6-6]

## Promoted From Short-Term Memory (2026-05-23)

<!-- openclaw-memory-promotion:memory:memory/2026-05-11.md:1:40 -->
- --- date: 2026-05-11 title: Workflow Automation Gap Fixed tags: [automation, cron, improvement, critical] projects: [infrastructure] summary: Identified and fixed critical gap - all automation scripts existed but weren't scheduled in cron --- ## Issue Identified Reviewed workspace automation and discovered a **critical gap**: - All automation scripts were documented and functional (`daily-status-summary.sh`, `health-quick-check.sh`, `snapshot.sh`, `weekly-safety-audit.sh`) - BUT no cron jobs were actually scheduled to run them - `crontab -l` returned empty despite AGENTS.md referencing automated daily snapshots, blog checks, and weekly audits - Result: System had zero automated maintenance running ## What Was Created ### `HOST-SETUP-CRON.sh` New idempotent cron setup script that: - Installs 7 essential automation jobs on the Hostinger VPS host - All jobs use `docker exec` to run inside the OpenClaw container - Automatically skips duplicate installations (safe to run multiple times) - Backs up existing crontab before making changes - Logs all actions to `memory/cron-install.log` **Installed Jobs:** - 02:30 UTC: Daily snapshot (GitHub backup) - 03:00 UTC: Daily health quick-check - 04:00 UTC: Curl domain validation - 05:00 UTC: Backup validation - 12:00 UTC: Daily status summary - 14:00 UTC (Mon/Wed/Fri): Blog post validation - 10:00 UTC (Sundays): Weekly safety audit ## Documentation Updates - Updated `scripts/README.md` with new cron setup section - Added critical setup warning to AGENTS.md - Documented proper container/host cron distinction ## Impact [score=0.885 recalls=5 avg=0.643 source=memory/2026-05-11.md:1-40]
