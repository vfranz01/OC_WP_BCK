...
# Validate critical rules in separate script
if [ -f "$WORKSPACE_DIR/scripts/validate_critical_rules.sh" ]; then
...
if [ "$VALIDATION_PASSED" = true ]; then
  ...
  echo "VALIDATION_OK" > "$WORKSPACE_DIR/memory/.validation_status"
  exit 0
else
  log "Validation FAILED. Triggering emergency snapshot for recovery..."
  echo "VALIDATION_FAILED" > "$WORKSPACE_DIR/memory/.validation_status"
  if [ -x "$SNAPSHOT_SCRIPT" ]; then
