...
### 1. Backup Validation
Automatically validate backup and trigger snapshot if needed:
```bash
# Validate backup and trigger snapshot if needed
...
### 2. Create backup_incidents.log if it doesn't exist
```bash
touch /home/node/.openclaw/workspace/memory/backup_incidents.log
```

### 3. Ensure backup validation script is executable
```bash
