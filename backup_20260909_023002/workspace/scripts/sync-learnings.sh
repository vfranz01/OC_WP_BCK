...
# Year-agnostic pattern (YYYY-MM-DD.md) so the scan keeps working after year rollover.
RECENT_FILES=$(find "$MEMORY_DIR" -maxdepth 1 -name "[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9].md" -type f -mtime -$DAYS_BACK 2>/dev/null | sort -r | head -20)
...
# Create temporary output for findings
{
  ...
  # NOTE: previously the "none found" branches were dead code — grep was piped
  # to head, so $? reflected head's exit (always 0), and the `|| echo` after
  # `fi` never fired. Capture matches and branch on emptiness instead.
  echo "## Potential Improvements Found:"
  echo "   Searching for explicit timestamps..."
  if matches="$(grep -h '^- \*\*2026' $RECENT_FILES 2>/dev/null | head -20)" && [ -n "$matches" ]; then
    echo "$matches"
  else
    echo "   (none with explicit timestamps)"
  fi

  # Scan for tagged improvements in markdown
  echo ""
  echo "## Items tagged [improvement]:"
  echo "   Searching for improvements..."
  if matches="$(grep -h '\[improvement\]' $RECENT_FILES 2>/dev/null | head -10)" && [ -n "$matches" ]; then
    echo "$matches"
  else
    echo "   (none found)"
  fi
  ...
  echo "   Searching for bug fixes or critical items..."
  if matches="$(grep -h '\[bug-fix\]\|\[critical\]' $RECENT_FILES 2>/dev/null | head -10)" && [ -n "$matches" ]; then
    echo "$matches"
  else
    echo "   (none found)"
  fi
