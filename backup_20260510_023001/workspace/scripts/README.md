...
├── weekly-safety-audit.sh (NEW: comprehensive weekly checks)
│   ├── validate_critical_rules.sh (verify MEMORY.md rules intact)
│   ├── File permission checks
│   ├── validate_backup.sh (backup health)
│   └── Memory structure validation
└── Logs to: memory/weekly-safety-audit.log

Manual/Diagnostic
├── restore.sh (emergency restore only)
├── backup_log_summary.sh (quick log summary)
├── rotate_backup_log.sh (automatic rotation)
├── check_validation_status.sh
├── validate_blog_job.sh (blog cron job monitoring)
└── backup_monitor.sh
```
