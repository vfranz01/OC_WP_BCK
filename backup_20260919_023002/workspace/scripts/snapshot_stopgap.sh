...
# ensure_daily_memory.sh and daily-status-summary.sh can be invoked by
# overlapping sessions.  Without a process lock, both can observe a stale
# snapshot and create duplicate multi-megabyte archives at the same time.
# flock releases the lock automatically when this shell exits.
LOCK_FILE="${TMPDIR:-/tmp}/openclaw-snapshot-stopgap.lock"
...
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
    log "⏭️ snapshot_stopgap: another run is already checking the snapshot — skipping"
    exit 0
  fi
else
  # Linux images normally provide flock; keep the stopgap functional on
  # minimal images rather than making a missing optional utility fatal.
  log "⚠️ snapshot_stopgap: flock unavailable — concurrency guard disabled"
fi
